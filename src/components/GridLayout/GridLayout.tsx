...
interface GridLayoutProps extends GridLayoutOptions, Pick<ViewportProps, 'onDrag' | 'className'> {
  children?: ReactNode;
  guideUi?: boolean;
}

const GridLayout = forwardRef<HTMLDivElement, GridLayoutProps>(function GridLayout ({ guideUi = false, gridSize: propGridSize = [40, 40], columns = [40, 16], gap = 8, children, type, ...props }, ref) {
  const [viewportSize, setViewportSize] = useState<Size>(INITIAL_SIZE);
  const [gridSize, setGridSize] = useState<Size>(INITIAL_SIZE);

  const layout = useLayout(
    LayoutManager,
    useMemo(() => ({ gridSize, gap, type }), [propGridSize[0], propGridSize[1], gap]),
  ) as LayoutManager;
